...
def main(ass_fasta_a, ass_fasta_b, assembly_file_a, assembly_file_b, 
         ...
         min_len=200, primer_file_filter='V', max_mismatches=4, nthreads=4):
    ...
    log('  Maximum number of primer mismatches: '+str(max_mismatches)+' nt')
    log('  Load files: ', end='')
    ...
    for primer_file in primer_files:
        ...
        log('OK.', end=True, time_stamp=False)
        log('    best...', end=False, time_stamp=True)
        ...
        # Select only alignments with up to max_mismatches 
        # blast_out_best.head()
        blast_out_best = blast_out_best.loc[blast_out_best[4] <= max_mismatches]
        ...
        #blast_out_best2a = blast_out_best2[blast_out_best2['mismatches'] >= 4]
        ...
        log('OK.', end=True, time_stamp=False)
        log('    v-regions...', end=False, time_stamp=True)        
        ...
        for meta, seq in ass_to_vreg_dict.items():
            ...
            if seq not in assid_to_vreg_dict[assid]:
                ...
                assid_to_vreg_dict[assid][seq] += 1
        log('OK.', end=True, time_stamp=False)
        log('    write.', end=False, time_stamp=True)
...
def parse_input():
    ...
    parser.add_argument('--max-mismatches', required=False, default=4,
                       help='Maximum number of mismatches for primer alignment.')
...
if __name__ == '__main__':
    ...
    main(ass_fasta_a, ass_fasta_b, assembly_file_a, assembly_file_b, primer_dir, 
         ...
         overhang=overhang, primer_file_filter=filter, max_mismatches=max_mismatches, nthreads=nthreads)
