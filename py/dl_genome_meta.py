...
import os, sys, datetime, re, argparse, logging
import pandas as pd
from six.moves import urllib

script_title = "Download Genome Metadata"
...
def parse_input():
	...
	parser.add_argument(
		...
		help="Comma delimited list of kingdoms (no spaces)."
	)
	args = parser.parse_args()
	return args

def download_assembly_summary(kingdom, output_path):
	""" Download a single assembly summary. If the target folder does not
	exist, create it. """
	asssum_base_url = 'ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/'
	asssum_full_url = f'{asssum_base_url}/{kingdom}/assembly_summary.txt'

	if not os.path.isdir(output_path):
		os.makedirs(output_path)
	
	output_file = os.path.join(output_path, f'{kingdom}_assembly_summary.txt')
	urllib.request.urlretrieve(asssum_full_url, output_file)
...
if __name__ == '__main__':
	...
	log.info(script_title)

	args = parse_input()
	log.info('Output path: ', args.output_path)
	log.info('Kingdoms: ', args.kingdoms)
	...
	for kingdom in kingdoms:
		log.info('Processing {kingdom}')
		
		# Download assembly summary
		log.info('Downloading assembly summary table...')
